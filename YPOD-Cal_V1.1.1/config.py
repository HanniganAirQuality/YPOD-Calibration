...
# ****************************************************************************
# ****************************************************************************
save_dir = "/Users/Chiara/Documents/Documents_Chiara_MacBook_Air/Research/YPOD Calibration/Model Improvement" # directory for write files
pod_id = ["U4", "U8", "X0", "F2", "F4", "D4", "Z2", "Z3", "U7", "U2", "U1", "A1", "T4", "O1", "L1", "B8", "G2"] # pods to look at
pod_id = ["K2", "T6", "X1", "U3", "V6"]
...
start_cutoff = pd.to_datetime("2025-09-01 16:30:00", format = "mixed") # start time for data window
end_cutoff = pd.to_datetime("2025-09-11 16:00:00", format = "mixed") # end time for data window
filename_date = "_2025_09_01.CSV"
...
ref2_filename = "DAQ_8_26_2025.CSV" # reference file name
ref2_headers = ["Timestamp (UTC)", "Timestamp", "CO_ppm", "CO2_ppm"] # referance file headers
ref2_row_drop = 1 # reference row to drop / ignore, include header rows
ref2_col_drop = ["Timestamp (UTC)"] # reference columns to drop / ignore
ref2_data = ["CO_ppm", "CO2_ppm"] # desired headers / data to look at and calibrate to
...
# co2_regr_var = ["CO2", "Temperature", "Relative Humidity"]
# co_regr_var = ["CO", "Temperature", "Relative Humidity"]
co2_regr_var = ["CO2", "T_CO2", "CO2sq", "sqrtCO2", "one_CO2", "Tsq"]
co_regr_var = ["CO2_CO", "COsq", "sqrtCO", "logCO", "CO", "CO2CO", "rHsq"]
# co2_regr_var = ["CO2", "Relative Humidity", "Temperature", "TrH", "TCO2",
#                 "CO2rH", "CO2CO", "T_rH", "T_CO2", "CO2_rH", "CO2_CO", 
#                 "Tsq", "rHsq", "CO2sq", "one_T", "one_rH", "one_CO2",
#                 "logT", "logrH", "logCO2", "eT", "erH", "sqrtT", 
#                 "sqrtrH", "sqrtCO2"] # co2 regression variables
# co_regr_var = ["CO", "Relative Humidity", "Temperature", "TrH", "TCO",
#                 "COrH", "CO2CO", "T_rH", "T_CO", "CO_rH", "CO2_CO", 
#                 "Tsq", "rHsq", "COsq", "one_T", "one_rH", "one_CO",
#                 "logT", "logrH", "logCO", "eT", "erH", "sqrtT", 
#                 "sqrtrH", "sqrtCO"] # carbon dioxide model
# "Tnorm", "rHnorm", "COnorm"
...
plt_colormap = "Wistia"
plt_style = "ggplot"
print_coefficients = False # prints the coefficients of the model individually
raw_pod_data_plotting = False # plots raw timeseries data from pods in pod_id array
model_accuracy_plotting = False # plots actual vs predicted model values for calibration variable (linear comparison)
model_timeseries_plotting = False # plots model timeseries data against referece timeseries data (timeseries comparison)
display_pod_data_timeseries = False # displays the pod timeseries data on the above plot
residual_plotting = False # plots individual residuals for a given raw signal
bias_plotting = False # box-and whiskers of residuals for bias visualization
z_score = False # z-scores data, needed for proper feature importance
coefficient_plotting = False # plots the magnitude of each coefficient with it's variable
feature_importace_plotting = False # plots charts for % importance for features
